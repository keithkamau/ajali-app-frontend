...
export const IncidentDetailPage = () => {
  ...
  const { currentIncident, statusHistory, loading, error, success } = useSelector((state) => state.incidents);
  ...
  // If we have an error and no current incident, show the error
  ...
  // If we have a current incident, render it even if there's an error (fallback data)
  if (!currentIncident) {
    return (
    ...
    <div className="incident-detail-page">
      ...
      <div className="incident-detail-header">
        <div>
          ...
          <div className="incident-detail-meta">
            ...
            <span className={`status-badge ${getStatusBadgeClass(currentIncident.status)}`}>
      ...
      <div className="incident-detail-content">
        <div className="card incident-detail-card">
          <h3 className="heading-4" style={{ marginBottom: '0.5rem' }}>Description</h3>
          <p className="body-text">{currentIncident.description}</p>
        </div>

        {currentIncident.location_lat && currentIncident.location_lng && (
          <div className="card incident-detail-card">
            <h3 className="heading-4" style={{ marginBottom: '0.5rem' }}>Location</h3>
        ...
        {currentIncident.images && currentIncident.images.length > 0 && (
          <div className="card incident-detail-card">
            <h3 className="heading-4" style={{ marginBottom: '0.5rem' }}>Images</h3>
        ...
        {currentIncident.videos && currentIncident.videos.length > 0 && (
          <div className="card incident-detail-card">
            <h3 className="heading-4" style={{ marginBottom: '0.5rem' }}>Videos</h3>
            <div className="incident-detail-media">
        ...
        {isAdmin && statusHistory && statusHistory.length > 0 && (
          <div className="card incident-detail-card">
            <h3 className="heading-4" style={{ marginBottom: '0.5rem' }}>Status History</h3>
            <div className="status-timeline">
              {statusHistory.map((entry, index) => (
                <div key={index} className="status-timeline-item">
                  <div className="status-timeline-badge">
                    <span className={`status-badge ${getStatusBadgeClass(entry.new_status)}`}>
                  ...
                  <div className="status-timeline-content">
                    ...
                    <div className="status-timeline-meta">
                      <span className="body-small text-muted">
                        By {entry.changed_by || 'Admin'} at {formatDate(entry.changed_at)}
                      </span>
                    </div>
                  </div>
                </div>
